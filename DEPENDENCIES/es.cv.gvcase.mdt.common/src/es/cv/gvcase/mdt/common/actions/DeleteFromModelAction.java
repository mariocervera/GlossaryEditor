 ...
 * Mario Cervera Ubeda (Prodevelop) - Allow deletion of editparts without semantic element
 *
 ******************************************************************************/
...
public class DeleteFromModelAction extends
		org.eclipse.gmf.runtime.diagram.ui.actions.DeleteFromModelAction {
	...
	protected Command getCommand(Request request) {
					...
					if (source != null && target != null) {
						return editPart
								.getCommand(new EditCommandRequestWrapper(
										new DestroyReferenceRequest(source,
												null, target, false)));
					}
				}
			}
		...
		if (elementsToDelete.size() > 0) {
			Command c = DeleteFromModelCommandRegistry
					.findCommand(getStructuredSelection());
			if (c != null)
				return c;

			return new ICommandProxy(new DeleteCommand(getEditingDomain(),
					"Delete elements", (List) null, elementsToDelete));
		}
	...
	protected boolean calculateEnabled() {
		...
		if (!enabled) {
			// Even though the selected elements don't have an associated
			// semantic element, they
			// may be deleted anyway if the edit part knows how to treat a
			...
			while (editParts.hasNext()) {
				...
				if (source != null && target != null) {
					Command command = ep
							.getCommand(new EditCommandRequestWrapper(
									new DestroyReferenceRequest(source, null,
											target, false)));
					if (command != null)
						enabled = true;
				}
		...
		return enabled
				& !DeleteFromModelAndDiagramBlackListRegistry.getInstance()
						.removeDeleteFromModelAction(getOperationSet());
	}
