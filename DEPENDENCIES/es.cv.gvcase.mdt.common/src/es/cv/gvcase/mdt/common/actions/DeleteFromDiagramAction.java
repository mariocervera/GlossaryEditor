...
/**
 * An override of the GMF DeleteFromDiagramAction so that it is available in
 * synchronized diagram editors. This <IAction>, when executed, deletes the
 * selected top most <View> of the selected <View> that shares the same
 * <EObject> and removes its reference from the <Diagram>. For example, if a
 * name showing label that is a child of a node is selected, the entire node
 * will be deleted, instead of only the label. It is not meant to be used but in
 * MOSKitt editors.
 * 
...
public class DeleteFromDiagramAction
	...
	/**
	 * The returned <Command> will delete the top most <View> of the selected
	 * <View> that share the same underlying <EObject>.
	 * 
	 * @return the command
	 */
	@Override
	protected Command getCommand() {
		...
		for (Iterator iter = objects.iterator(); iter.hasNext();) {
			...
			if (editPart instanceof IGraphicalEditPart) {
				...
				EditPart editPartToDelete = (EditPart) ((IGraphicalEditPart) editPart)
						.getViewer().getEditPartRegistry().get(view);
				Command command = editPartToDelete
						.getCommand(createDeleteFromDiagramRequest());
				if (command != null) {
	...
	protected boolean calculateEnabled() {
		// marc:: check if exists an extension point that removes this action
		return super.calculateEnabled()
				&& !DeleteFromModelAndDiagramBlackListRegistry.getInstance()
						.removeDeleteFromDiagramAction(getOperationSet());
	}
	...
	protected View getViewToDelete(IGraphicalEditPart editPart) {
		...
		// Modified by gmerin
		// A NodeEditPart might be composed by more than one EditPart (like
		// LabelEditParts, etc). In the cases were the user selects a part and
		// not the node, what we want to achieve is to return the view of the
		// Node's EditPart, not the view of the selected EditPart.
		if (!isNodeEditPart(editPart)) {
	...
	/**
	 * Code to check if the passed EditPart is a Node EditPart.
	 * 
	 * @author gmerin
	 * @param editPart
	 * @return true if editPart is instance of Shape/Connection NodeEditPart
	 */
	public boolean isNodeEditPart(IGraphicalEditPart editPart) {
	 ...
	 */
	private boolean checkShareSameEObject(IGraphicalEditPart editPart1,
			IGraphicalEditPart editPart2) {
	...
	public void refresh() {
		super.refresh();
		/*
		 * Disable the action if any selected editpart returns an unexecutable
		 * command for our request
		 */
