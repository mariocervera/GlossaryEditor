...
public class ReconnectToIntersectionPoints extends Command {
	 ...
	 */
	public ReconnectToIntersectionPoints(
	...
	public void execute() {
		...
		ViewDescriptor viewDescriptor = createRequest
		...
		// Obtain the EditPart
		ConnectionEditPart connEP = (ConnectionEditPart) viewer
		...
		// Obtain the commands
		compoundCmd.add(getCommand(sourceRequest, (GraphicalEditPart) connEP
				.getSource()));
		compoundCmd.add(getCommand(targetRequest, (GraphicalEditPart) connEP
				.getTarget()));
	 ...
	 * 
	 * @param connEP the conn ep
	 * @param isSource the is source
	 * 
	 * @return the reconnect request
	 */
	public static ReconnectRequest buildReconnectRequest(
			ConnectionEditPart connEP, boolean isSource) {
	 ...
	 */
	public static Point getIntersectionPoint(ConnectionEditPart connEP,
			boolean source) {
	 ...
	 * 
	 * @param request the request
	 * @param graphicalEP the graphical ep
	 * 
	 * @return the command
	 */
	public static Command getCommand(Request request,
			GraphicalEditPart graphicalEP) {
		return graphicalEP.getEditPolicy(EditPolicy.GRAPHICAL_NODE_ROLE)
				.getCommand(request);
	}
	 ...
	 * Modify location.
	 * 
	 * @param reconnectRequest the reconnect request
	 */
	public static void modifyLocation(ReconnectRequest reconnectRequest) {
		...
		sourceEP = (ShapeNodeEditPart) reconnectRequest.getConnectionEditPart()
				.getSource();
		targetEP = (ShapeNodeEditPart) reconnectRequest.getConnectionEditPart()
				.getTarget();
