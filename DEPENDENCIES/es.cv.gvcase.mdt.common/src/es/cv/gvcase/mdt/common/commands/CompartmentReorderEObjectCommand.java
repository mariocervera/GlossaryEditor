 ...
 */
public class CompartmentReorderEObjectCommand extends
	 ...
	 */
	public CompartmentReorderEObjectCommand(
			TransactionalEditingDomain editingDomain, String label,
			EList<EObject> elements, EditPart childEditPart,
			EditPart afterEditPart) {
		super(editingDomain, label, getWorkspaceFiles(((View) childEditPart
				.getModel()).getElement()));
	...
	protected CommandResult doExecuteWithResult(
		...
		try {
			...
			View afterView = afterEditPart == null ? null
					: (View) afterEditPart.getModel();
			EObject afterElement = afterView == null ? null : afterView
					.getElement();

			// 1. Reorder the movedElement in the domain model
			int newSemanticPosition = afterElement == null ? elements.size() - 1
					: elements.indexOf(afterElement) > elements
							.indexOf(movedElement) ? elements
							.indexOf(afterElement) - 1 : elements
			...
			int newViewPosition = afterView == null ? views.size() - 1 : views
					.indexOf(afterView) > views.indexOf(movedView) ? views
					.indexOf(afterView) - 1 : views.indexOf(afterView);
			ViewUtil.repositionChildAt((View) childEditPart.getParent()
					.getModel(), (View) childEditPart.getModel(),
					newViewPosition);
