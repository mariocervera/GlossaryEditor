...
/**
 * A DragAndDropEditPolicy for list compartment nodes. Checks whether the
 * dropped elements can be shown as views in the compartment and if so, created
 * their views and add their references to the diagram's list of references.
 * Those elements containers are changed to the element represented by the host
 * of the policy.
 * 
...
public class ListDragDropEditPolicy extends DragDropEditPolicy {
	 ...
	 * 
	 * @see
	 * es.cv.gvcase.mdt.common.edit.policies.DragDropEditPolicy#buildDropCommand
	...
	protected Command buildDropCommand(DropObjectsRequest request,
		...
		if (storableElements != null && storableElements.size() > 0) {
			...
			if (storablesCommand.canExecute()) {
				command = command == null ? storablesCommand : command
						.chain(storablesCommand);
			}
		}
		...
		command = setEContainerCommand != null ? command
				.chain(setEContainerCommand) : command;
		// build create views
	...
	protected Command createAddToNewContainerCommand(List<EObject> elements) {
		...
		CompositeCommand cc = new CompositeCommand(
				"Set elements parent container");
		for (EObject element : elements) {
			EStructuralFeature feature = resolver
					.getEStructuralFeatureForEClass(element.eClass());
			if (feature != null) {
