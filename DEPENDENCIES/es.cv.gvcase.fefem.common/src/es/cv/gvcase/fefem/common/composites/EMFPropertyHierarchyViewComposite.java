...
/**
 * Creates a readonly tree composite very usefull for creating the master part
 * of a master/detail UI patterns. This composite, as
 * EMFContainedHierarchicalCollectionEditionComposite does, is based on ecore
 * Structural Features in which the root elements of the tree are binded to a
 * given feature and the parent/child reflexive relation is determined by
 * another given reflexive feature.
 * 
...
public abstract class EMFPropertyHierarchyViewComposite extends
	 ...
	 * @param page
	 */
	public EMFPropertyHierarchyViewComposite(Composite parent, int style,
	...
	protected void bindFeatureToWidget() {
		...
		IObservableFactory fac = new IObservableFactory() {
			...
			public IObservable createObservable(Object target) {
				...
				if (target instanceof IObservableList) {
					...
					IObservableList childrenModelObservable = EMFObservables
							.observeList((EObject) target,
									getReflexiveFeature());
		...
		if (!isDetailComposite()) {
			...
			candidatesObservable = EMFObservables.observeList(
					this.getEObject(), this.getFeature());
			...
			candidatesObservable = EMFObservables.observeDetailList(Realm
					.getDefault(), selectionObservable, this.getFeature());
