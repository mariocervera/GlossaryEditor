...
public abstract class EMFContainedCollectionEditionComposite extends
	...
	protected void createWidgets(FormToolkit toolkit) {
		...
		if (canFilter()) {
			searchLabel = toolkit
					.createLabel(
							this,
							Messages.EMFContainedCollectionEditionComposite_Filter + ':');
		...
		Table table = toolkit.createTable(this, getStyle());
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (viewer.getSelection() instanceof StructuredSelection) {
					...
					if (elementsToDelete.size() > 0) {
						...
						// Support for other element types
						// EObject eObject = (EObject) elementsToDelete.get(0);
						Object eObject = elementsToDelete.get(0);
						...
						if (modelObservable.size() > 1) { // If we have more
							...
							// previous
							// sibling
							? new StructuredSelection(modelObservable
									.get(pos - 1))
									: new StructuredSelection(modelObservable
											.get(pos + 1)); // otherwise, we
							// will select the
							// next one
						}

						EStructuralFeature feature = getFeature();
						if (feature instanceof EReference // If it's a
								// containment
								// reference we
								// delete the
								// elements from the
							...
							getEditingDomain().getCommandStack().execute(
									DeleteCommand.create(getEditingDomain(),
											elementsToDelete));
						} else { // otherwise, we just remove the elements from
							// the obsevableList
			...
			public void widgetSelected(SelectionEvent event) {
				if (getViewer().getSelection() instanceof StructuredSelection
					...
					if (elementToMove != null) {
						int oldIndex = ((IObservableList) modelObservable)
								.indexOf(elementToMove);
						int newIndex = oldIndex == ((IObservableList) modelObservable)
			...
			if (sorter != null) {
				column.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						...
						for (int i = 0; i < getViewer().getTable()
								.getItemCount(); i++) {
						...
						if (!elements.equals(orderedElements)) {
							...
							((IObservableList) modelObservable)
	...
	protected void bindFeatureToWidget() {
		...
		// Observes the containedTerms list to get automatic refresh of the
		// TableViewer.
		// It will also allow to add and remove elements from the list without
		// using commands.
		...
		if (!isDetailComposite()) {
			...
			modelObservable = EMFEditObservables.observeList(getPage()
					.getEditor().getEditingDomain(), this.getEObject(),
					getFeature());
			...
			modelObservable = EMFEditObservables.observeDetailList(Realm
