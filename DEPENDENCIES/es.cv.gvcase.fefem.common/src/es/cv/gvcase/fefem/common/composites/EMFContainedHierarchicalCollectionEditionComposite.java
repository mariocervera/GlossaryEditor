...
public abstract class EMFContainedHierarchicalCollectionEditionComposite extends
		EMFPropertyComposite {

	protected static final int ONECOLUMN_MODE = 0;
	protected static final int MULTICOLUMN_MODE = 1;
	...
	protected Composite buttonsContainer;
	...
	public EMFContainedHierarchicalCollectionEditionComposite(Composite parent,
			int style, FormToolkit toolkit, EObject eobject, FEFEMPage page) {
	...
	protected void createAddButton(Composite container, String buttonLabel,
			FormToolkit toolkit) {
		// Add button
		Button addButton = toolkit.createButton(container, buttonLabel,
				SWT.NONE);
	...
	protected void createAddChildButton(Composite container,
			String buttonLabel, FormToolkit toolkit) {
		// AddChild button
		Button addChildButton = toolkit.createButton(container, buttonLabel,
				SWT.NONE);
	...
	protected void createRemoveButton(Composite container, String buttonLabel,
			FormToolkit toolkit) {
		// Remove button
		Button removeButton = toolkit.createButton(container, buttonLabel,
				SWT.NONE);
	...
	protected void createExpandButton(Composite container, String buttonLabel,
			FormToolkit toolkit) {
		// Expand button
		Button expandButton = toolkit.createButton(container, buttonLabel,
				SWT.NONE);
	...
	protected void createCollapseButton(Composite container,
			String buttonLabel, FormToolkit toolkit) {
		// Collapse button
		Button collapseButton = toolkit.createButton(container, buttonLabel,
				SWT.NONE);
	...
	protected void createUpButton(Composite container, String buttonLabel,
			FormToolkit toolkit) {
		// Up button
		Button upButton = toolkit
				.createButton(container, buttonLabel, SWT.NONE);
	...
	protected void createDownButton(Composite container, String buttonLabel,
			FormToolkit toolkit) {
		// Down button
		Button downButton = toolkit.createButton(container, buttonLabel,
				SWT.NONE);
		...
		SelectionAdapter adapter = new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if (getViewer().getSelection() != null) {
					Object selection = ((StructuredSelection) getViewer()
					...
					if (selection instanceof EObject) {
						...
						if ((o != null) && (child != null)) {
							...
							getEditingDomain().getCommandStack().execute(
									AddCommand.create(getEditingDomain(), o,
											getReflexiveFeature(), child));
							getPage().setDirty(true);
	...
	/**
	 * Creates the a SelectionListener which will invoke the code to
	 * remove/delete a selected element from the tree viewer.
	 * 
		...
		SelectionAdapter adapter = new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if (viewer.getSelection() instanceof TreeSelection) {
					TreeSelection selection = (TreeSelection) viewer
							.getSelection();
					List<?> elementsToDelete = selection.toList();
					if (elementsToDelete.size() > 0) {
						// We prepare the next selection, based on the element
						// to delete
						...
						;
						if (path.getSegmentCount() == 1) { // If it is a first
							// level element, we
							// will select a
							// sibling element
							if (modelObservable.size() > 1) { // If we have more
								// than one
								// element
								int pos = modelObservable.indexOf(eObject);
								nextSelection = (pos == modelObservable.size() - 1) // If
								// it's
								// the
								// last
								// first
						...
						} else { // If it is not a first level element, we will
							// select its parent element
							nextSelection = new TreeSelection(path
									.getParentPath());
						}

						EditingDomain domain = getEditingDomain();
						domain.getCommandStack().execute(
								DeleteCommand.create(domain, elementsToDelete));
						getPage().setDirty(true);
		...
		if (getViewer().getSelection() instanceof StructuredSelection) {
			...
			if (elementToMove instanceof EObject) {
				...
				if (!isChild((EObject) elementToMove)
					...
					IObservableList childrenModelObservable = EMFEditObservables
							.observeList(getPage().getEditor()
									.getEditingDomain(),
									((EObject) elementToMove).eContainer(),
									getReflexiveFeature());
					if (childrenModelObservable != null) {
						oldIndex = childrenModelObservable
								.indexOf(elementToMove);
						if (up) {
							newIndex = oldIndex == 0 ? oldIndex : oldIndex - 1;
						} else {
							newIndex = oldIndex == childrenModelObservable
									.size() - 1 ? oldIndex : oldIndex + 1;
						}
						if (oldIndex != newIndex) {
	...
	protected void createAddAndRemoveButtons(Composite container,
		...
		createAddButton(container, getButtonLabel(ADDROOTELEMENT_BUTTON_ID),
				toolkit);

		createAddChildButton(container,
				getButtonLabel(ADDCHILDELEMENT_BUTTON_ID), toolkit);
	...
	protected void createExpandAndCollapseButtons(Composite container,
		...
		createCollapseButton(container, getButtonLabel(COLLAPSE_BUTTON_ID),
				toolkit);
	...
	protected String getButtonLabel(int buttonID) {
		switch (buttonID) {
		case ADDROOTELEMENT_BUTTON_ID:
			return Messages.EMFContainedHierarchicalCollectionEditionComposite_NewRoot;
		case ADDCHILDELEMENT_BUTTON_ID:
			return Messages.EMFContainedHierarchicalCollectionEditionComposite_NewChild;
		case REMOVEELEMENT_BUTTON_ID:
			return Messages.EMFContainedHierarchicalCollectionEditionComposite_Remove;
		case EXPAND_BUTTON_ID:
			return Messages.EMFContainedHierarchicalCollectionEditionComposite_Expand;
		case COLLAPSE_BUTTON_ID:
			return Messages.EMFContainedHierarchicalCollectionEditionComposite_Collapse;
		case UP_BUTTON_ID:
	...
	@Override
	public int getStyle() {
		return getStyle(getMode());
	}

	protected int getStyle(int mode) {
		if (mode == ONECOLUMN_MODE)
			return SWT.BORDER | SWT.FULL_SELECTION | SWT.SINGLE;
		else
			return SWT.BORDER | SWT.FULL_SELECTION | SWT.SINGLE | SWT.H_SCROLL
					| SWT.V_SCROLL;
	}

	protected int getMode(String[] columnNames) {
		if ((columnNames == null) || (columnNames.length == 0))
			return ONECOLUMN_MODE;
		else
			return MULTICOLUMN_MODE;
	}

	protected int getMode() {
		return getMode(getColumnNames());
	}

	/**
	 * EventType for Search textcontrol used in filter mode. Can be changed by
	 * descendants
	 * 
	 * @return
	 */
	protected int getFilterEventType() {
		return SWT.Modify;
	}
	...
	protected void createWidgets(FormToolkit toolkit) {
		...
		if (canFilter()) {
			searchLabel = toolkit
					.createLabel(
							this,
							Messages.EMFContainedHierarchicalCollectionEditionComposite_Filter);
			...
			searchText.addListener(getFilterEventType(), new Listener() {
		...
		Tree tree = null;
		if (getMode(columnNames) == ONECOLUMN_MODE)
			tree = toolkit.createTree(this, getStyle());
		else {
			// Multicolumn support
			tree = new Tree(this, getStyle());
	...
	/**
	 * This method lets the descendants to customize every created TableColumn
	 * when more than one column is used for table
	 * 
	 * @param c
	 */
	...
	protected void bindFeatureToWidget() {
		...
		if (!isDetailComposite()) {
			...
			modelObservable = EMFEditObservables.observeList(getPage()
					.getEditor().getEditingDomain(), this.getEObject(), this
					.getFeature());
			...
			modelObservable = EMFEditObservables
					.observeDetailList(Realm.getDefault(), this.getPage()
							.getEditor().getEditingDomain(),
							selectionObservable, this.getFeature());

		}

		EMFObservableTreeFactory fac = new EMFObservableTreeFactory(
				getReflexiveFeature(),
				getPage().getEditor().getEditingDomain(), modelObservable);
		EMFTreeStructuredAdvisor adv = new EMFTreeStructuredAdvisor(
				getReflexiveFeature());
		ObservableListTreeContentProvider contentProvider = new ObservableListTreeContentProvider(
				fac, adv);
		getViewer().setContentProvider(contentProvider);

		// Hey!!, pay attention to this block. For some reason it is required to
		// pass a different input object instance to AbstractTreeViewer for a
		// normal mode or master/detail mode. If we pass always the already
		// resolved modelObservable, then the TreeViewer does not work fine.
		if (!isDetailComposite())
	...
	protected boolean isChild(EObject eobj) {
	...
	protected List<Control> getWidgetsList() {
		...
		list.add(searchText);
		list.addAll(addButtons);
		list.addAll(removeButtons);
		list.addAll(treeButtons);
		list.add(upButton);
		list.add(downButton);
		list.add(treeViewer.getTree());
		return list;
	}
	...
	/**
	 * public access to the reflexive feature
	 * 
	 * @return
	 */
	public EStructuralFeature getReflexiveEStructuralFeature() {
		return getReflexiveFeature();
	}
