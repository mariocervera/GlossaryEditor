		...
		if(this.domain != null){
			...
			tableViewer.setLabelProvider(new CellLabelProvider(){
				...
				public void update(ViewerCell cell) {
						...
						if(((Resource) cell.getElement()).getErrors().size()>0){
							Image img = getImageData("error.gif");
							if(img!=null)
								cell.setImage(img);
							
							Diagnostic d = ((Resource) cell.getElement()).getErrors().get(0);
							cell.getControl().setToolTipText(d.getMessage());
						}else{
							Image img = getImageData("check.gif");
							if(img!=null)
								cell.setImage(img);
						}
				...
				public Image getImageData(String resourcefilename) {
					String imagePath = "";
					try {
						imagePath = FileLocator.toFileURL(
								Platform.getBundle("es.cv.gvcase.fefem.common")
